...
class PointCloudVisualizer:
    ...
    def visualize(
        ...
        - Multiple clouds: uses uniform colors to distinguish clouds (unless ORIGINAL is specified)
        ...
        if n_clouds == 1:
            ...
            # Multi-cloud mode
            if color_scheme == ColorScheme.ORIGINAL:
                # Apply ORIGINAL to all clouds
                for i in range(n_clouds):
                    self._apply_color_scheme(i, ColorScheme.ORIGINAL)
            else:
                # Use uniform colors for distinction (original behavior)
    ...
    def create_animation_frames(
        ...
        # Apply colors based on cloud count and scheme
        n_clouds = len(self._point_clouds)
        if n_clouds == 1:
            self._apply_color_scheme(0, color_scheme)
        else:
            if color_scheme == ColorScheme.ORIGINAL:
                ...
                # Use uniform colors for distinction
                self._apply_multi_cloud_colors()
    ...
    def _apply_color_scheme(self, cloud_index: int, scheme: ColorScheme) -> None:
        ...
        Args:
            cloud_index: Index of the point cloud
            scheme: Color scheme to apply
        """
        pcd = self._point_clouds[cloud_index]

        # Skip if using ORIGINAL scheme and point cloud already has colors
        if scheme == ColorScheme.ORIGINAL and pcd.has_colors():
            return
    ...
    def _compute_colors(
        ...
        if scheme == ColorScheme.ORIGINAL:
            # Return existing colors if available, otherwise fallback to uniform
            pcd = self._point_clouds[cloud_index]
            if pcd.has_colors():
                return np.asarray(pcd.colors)
            else:
                # Fallback to uniform color if no original colors exist
                color = self._uniform_colors[cloud_index % len(self._uniform_colors)]
                return np.tile(color, (n_points, 1))

        elif scheme == ColorScheme.UNIFORM:
            color = self._uniform_colors[cloud_index % len(self._uniform_colors)]
            return np.tile(color, (n_points, 1))
