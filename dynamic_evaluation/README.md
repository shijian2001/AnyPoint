...
Dynamic Evaluation 的目标是在有限查询预算 `B` 下，更高效地发现被测 3D 多模态模型的错误样本、薄弱任务类型和能力边界。

候选池不是“每一轮重新生成的一批任务”，而是先由 AnyPoint 一次性合成一个固定候选池，并缓存到磁盘。后续动态策略和随机基线都从同一个固定池中抽样，因此两种策略面对的候选空间一致，比较更公平，也更容易复现实验。

核心思想是“探索 + 利用”：

- **利用**：优先评测与已知错误任务集合 `E` 相似的候选（像错题，更可能继续出错）。
- **探索**：避开与已答对任务集合 `C` 相似的候选（已掌握的区域重复评测收益低）。

也就是说，鼓励继续挖掘“像错题”的区域，同时主动避开模型已经答对的安全区域。
...
任务向量使用三组件编码，而不是把 layout、question、answer 简单拼接成一个字符串后编码。
...
每个候选任务 `t` 用两个余弦相似度信号打分：

```text
error  = max_sim(t, 错误集 E)    # 像已答错的任务 → 利用已知弱点
correct = max_sim(t, 正确集 C)    # 像已答对的任务
max_sim(t, S) = max_{s ∈ S} cos(v_t, v_s)   # 集合为空时取 0，结果裁剪到 [0,1]
```

我们想要“像错题、又不像已答对题”的任务。默认策略 **`tradeoff`** 把利用项 `error` 和探索项 `1 - correct` 各取指数后相乘，`λ ∈ [0, 1]` 控制两者权重：

```text
U(t) = error^(1-λ) · (1 - correct)^λ
```

- `error`（利用）：越像已知错题，越可能落在模型薄弱区域，应优先评测。
- `1 - correct`（探索）：越远离已答对区域越好，避免重复评测模型已掌握的样本。
- `λ`：`λ=0` 纯利用（只追错题），`λ=1` 纯探索（只避开正确区），默认 `0.2`。

两项相乘意味着任一项过低都会拉低整体分数（只像错题但也像已答对题、或远离正确区却完全不像错题，都不会被优先选）；指数权重保证 `U(t)` 落在 `[0,1]`。

实现还提供另外几种组合形式用于消融对比（由 `--strategies` 选择）：

```text
tradeoff_log : log U = (1-λ)·log(error) + λ·log(1-correct)   # 与 tradeoff 同序，数值更稳
linear       : U = error - λ·correct                          # 线性减
gated        : U = error · (1 - λ·correct)                     # 乘法门控
exploit_only : U = error                                    # 只利用：纯追错题
explore_only : U = log(1 - correct)                          # 只探索：纯避开正确区
```

## 4. 固定候选池与缓存机制

`pool_size = N` 表示一次性预生成的固定候选池总大小，而不是每轮重新生成的候选数量。
...
while |H| < B:
    for t in R:
        ...
        error  = max_sim(v_t, E)
        correct = max_sim(v_t, C)
        U(t) = error^(1-λ) * (1 - correct)^λ        # tradeoff (默认)
...
`compare_eval_strategies.py` 用于在同一个固定任务池上比较随机采样和动态采样。
...
因此，动态策略不只是追着错误样本做局部重复，而是在固定候选池中持续寻找“靠近错误区域、远离正确区域”的任务。这种机制能更快地发现模型在距离、尺寸、属性、频次和空间关系等维度上的失败模式，并把答错样本沉淀为可复用的 `hard_data`。

## 10. 关键设计要点

| 项目 | 设计 |
| --- | --- |
| 候选池 | 启动时生成固定池 `P`，每轮重排剩余池 `R_i` |
| pool_size 含义 | 固定候选池总大小 |
| 效用函数 | 默认 tradeoff：`error^(1-λ) · (1-correct)^λ`（另有 tradeoff_log / linear / gated / exploit_only / explore_only 供消融） |
| 探索参照（远离对象） | 已答对任务 `C` |
| 任务向量 | layout / question / answer 分别编码后拼接 |
| 对比实验 | 随机和动态共享同一固定候选池 |
| 可复现性 | 任务池带签名缓存，可复用、可追溯 |
| 输出 | random/dynamic 对比和 `compare_summary.json` |
